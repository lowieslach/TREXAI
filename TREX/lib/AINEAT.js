...
function NEAT(config) {
  ...
  //---------------------------LOWIE SLACHMUYLDERS-------------------------------------------
  this.getNeuralNetwork = function (index) {
    return this.creatures[index];
  };

  this.importOneModel = function (data) {
    if (JSON.stringify(data[0]) === JSON.stringify(this.exportModel)) {
      this.oneModel = new Creature(this.model);
      this.oneModel.setFlattenedGenes(data[1][0]);
    } else {
      throw "Invalid model!";
    }
  };

  this.getNeuralNetworkOneModel = function () {
    return this.oneModel;
  };

  this.getDesicionsOneModel = function () {
    return this.oneModel.desicion();
  };

  this.setInputsOneModel = function (array) {
    this.oneModel.setInputs(array);
  };

  this.feedForwardOneModel = function () {
    this.oneModel.feedForward();
  };
  ...
  this.desicion = function () {
    ...
    for (
      ...
      if (
        ...
        max = this.network.layers[this.network.layers.length - 1].nodes[i]
          .value;
        index = i;
      }
    }
